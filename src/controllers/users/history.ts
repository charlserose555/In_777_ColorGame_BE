...
export const list = async (req: Request, res: Response) => {
    const { pageSize = null, page = null, userId = null, country = null, date = null } = req.body;
    let query = {} as any;
    if (userId) {
    ...
    if (!pageSize || !page) {
        const results = await LoginHistories.aggregate([{ $match: query }, ...aggregateQuery]);
...
export const updateOne = async (req: Request, res: Response) => {
    ...
    const result = await LoginHistories.aggregate([{ $match: query }, ...aggregateQuery]);
    return res.json(result[0]);
};
